...
int fpause=0;	   	   	// Do not force Pause Value.
int flbflb=1;	   	   	// Repeat File lastbit
...
float bpause;				// Value of the pause (in seconds)
...
if (argc<2)
	...
	printf("                  -pause n  Change pause value after blocks (in ms)\n");
	printf("                  -no2flb   Do not repeat file last bit\n");	 
	exit(0);
	}
for (n=1; n<argc; n++)
	{
	if (argv[n][0]=='-')
		...
		switch (argv[n][1])
			...
			case 'i':	if (strcmp(argv[n],"-ignore")) invalidoption(argv[n]);
					 	ignore=1; break;
			case 'n':	if (!strcmp(argv[n],"-noaprox")) {aprox=0;}
						else {
					 		if (strcmp(argv[n],"-no2flb")) invalidoption(argv[n]);
								flbflb=0;
							}
						break; 	      	  
			...
			case 'p':	if (!strcmp(argv[n],"-pilot")) { MIN_PILOT=getnumber(argv[n+1]); n++; }
						else {
							if (strcmp(argv[n],"-pause")) invalidoption(argv[n]);
								fpause=1;
								bpause=getnumber(argv[n+1])/1000.0; n++;
							}
					 	break;
